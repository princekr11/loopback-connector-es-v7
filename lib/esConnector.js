...
const { setupCluster } = require('./setupCluster');
...
class ESConnector {
  constructor(settings, dataSource) {
    ...
    this.configuration.ns = this.configuration.ns || 'vf';
    ...
    this.clusterSettings = settings.clusterSettings || {};
...
ESConnector.prototype.connect = function (callback) {
  ...
  if (self.db) {
    ...
    self.ping(() => {
      log('ESConnector.prototype.connect', 'setupCluster', 'Initiated');

      // Setting up luster with initial settings
      self.setupCluster()
        .then(() => {
          log('ESConnector.prototype.connect', 'setupCluster', 'finished');
          callback(null, self.db);
        })
        .catch((err) => {
          log('ESConnector.prototype.connect', 'setupCluster', 'failed', err);
          callback(null, self.db);
        });
    });
  }
...
ESConnector.prototype.setupCluster = setupCluster;
/**
...
ESConnector.prototype.addDefaults = function (modelName, functionName) {
  ...
  if (modelClass && _.isObject(modelClass.settings.elasticsearch)) {
    filter.index = self.getIndexName(modelName);
    if (self.settings.version < 7) {
      filter.type = self.getTypeName(modelName);
    }
  }
...
/**
 * Fetch Index Name based on model. Append ns to index
 *
 * @param {object} modelClass
 * @returns {String} Filter with index and type
 */
ESConnector.prototype.getIndexName = function (modelName) {
  const self = this;
  let modelClass = self._models[modelName];
  return `${(modelClass.settings.elasticsearch.index || '')}_${self.configuration.ns}`;
};

/**
 * Fetch Type name based on model
 *
 * @param {object} modelClass
 * @returns {String} Filter with index and type
 */
ESConnector.prototype.getTypeName = function (modelName) {
  const self = this;
  let modelClass = self._models[modelName];
  return `${(modelClass.settings.elasticsearch.type || '')}_${self.configuration.ns}`;
};
 ...
 * @param {Function} [cb] The callback function
 */
ESConnector.prototype.autoupdate = autoupdate;
